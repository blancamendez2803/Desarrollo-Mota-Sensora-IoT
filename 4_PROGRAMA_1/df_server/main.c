                ...
                if (ctx->recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
                    if(onoff == 1){
                        if (ctx->recv_cred == ESP_BLE_MESH_DIRECTED_CRED) {
                            ...
                            ESP_LOGI(TAG, "Receive a message send by DF.");
                            ...
                            ESP_LOGI(TAG, "Receive a message send by flooding.");
                                         ...
                                         esp_ble_mesh_server_recv_gen_onoff_set_t *set)
{
    ...
    case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET:
        ...
        ESP_LOGI(TAG, "Send 0x%06x", ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS);
        break;
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    ESP_LOGI(TAG, " %s", param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ? "GEN_ONOFF_SET" : "VND_MODEL");
    switch (event) {
    case ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT");
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK) {
            ESP_LOGI(TAG, "onoff change evt 0x%02x", param->value.state_change.onoff_set.onoff);
            ESP_LOGI(TAG, "EL ESTADO DE LOS LED ES %s", param->value.state_change.onoff_set.onoff ? "ON" : "OFF");
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_RECV_GET_MSG_EVT:
        ...
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET) {
            ...
            ESP_LOGI(TAG, "onoff get msg 0x%02x", srv->state.onoff);
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_RECV_SET_MSG_EVT:
        ...
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK) {
            ESP_LOGI(TAG, "onoff set msg 0x%02x, tid 0x%02x", param->value.set.onoff.onoff, param->value.set.onoff.tid);
                                             ...
                                             esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT:
        if (param->model_operation.opcode == ESP_BLE_MESH_VND_MODEL_OP_SET) {
            ...
            ESP_LOGI(TAG, "recv msg %s", param->model_operation.msg);

            ESP_LOGI(TAG, "msg: ");
            for (int i = 0; i < len; i++) {
                ESP_LOGI(TAG, "%02X ", msg[i]);}

            ESP_LOGI(TAG, "ESP_BLE_MESH_VND_MODEL_OP_STATUS 0x%06x", ESP_BLE_MESH_VND_MODEL_OP_STATUS);
    ...
    case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
        ...
        ESP_LOGI(TAG, "MENSAJE  0x%06" PRIx32, param->model_send_comp.opcode);
        ESP_LOGI(TAG, "Send 0x%" PRIx32, param->model_send_comp.opcode);
        break;
    default:
        break;
    }
